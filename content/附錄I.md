...
請注意，`block`是一個語句規則，但在其它規則中也作為非終止符使用，用於表示函式體等內容。
...
詞法被掃描器用來將字元分組為詞法標識（token）。語法是[上下文無關](https://en.wikipedia.org/wiki/Context-free_grammar)的，詞法是[正則](https://en.wikipedia.org/wiki/Regular_grammar)的——注意這裡沒有遞迴規則。
